...
> fffnarediJust :: NewtonRaphson -> Maybe NewtonRaphson
> fffnarediJust x = Just x

> fffpomozna Nothing = Nothing
> fffpomozna (Just x) = Just (guess x)

> newton_raphson :: [Double] -> ([Result] -> [Result]) -> Maybe [Double]
> newton_raphson first_guess constraints = fffpomozna solution
>   where
>   nr = create_newton_raphson first_guess (constraints . variables)
>   iterations = filter converged $ take 30 $ iterate nr_iteration nr
>   solution = ((map fffnarediJust iterations) ++ (replicate 6 Nothing)) !! 5 --Nothing::(Maybe NewtonRaphson)) !! 5 --(iterations ++ error "Circle: Failed to converge") !! 5
>   --solution = ((map fffnarediJust iterations) ++ [Nothing]) !! 5 --Nothing::(Maybe NewtonRaphson)) !! 5 --(iterations ++ error "Circle: Failed to converge") !! 5
>   -- solution = ((map fffnarediJust iterations) ++ error "Circle: Failed to converge") !! 5
...
>   difference = value (badness nr - badness nr')
>   scale = max 0.1 (gradient / (2 * (gradient - difference)))
>   good_enough = difference >= 1e-3 * gradient
